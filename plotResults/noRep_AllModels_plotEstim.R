...
biasSummary <- read.csv("./results/noRep_AllModels_AllResults.csv") %>% 
  filter(N == 500, Convergence == 1) %>% 
  mutate(Convergence = as.factor((N_Rhat<= 1.1) * 1)) %>% 
  mutate(p = case_when(model == "classical" ~ p-0.015,
                       model == "noSingle" ~ p-0.005,
                       model == "LMMPt" ~ p+0.005,
                       model == "LMMqPtab" ~ p+0.015),
         model = as.factor(model),
         Convergence = fct_relevel(Convergence, "1", "0")) %>% 
  mutate(model = fct_recode(model, "Yoshizaki's" = "noSingle",
                            "Mta" = "LMMPt",
                            "Mtan" = "LMMqPtab",
                            "Mt" = "classical"))
biasSummaryMeans <- biasSummary %>% 
  group_by(model, p, alpha, S, Prior) %>% 
...
for(s in unique(biasSummary$S)){
  for(a in als){
    ...
    plts[[as.character(s)]][[as.character(a)]]  <- ggplot()+
      ...
      myThemePdf +
      scale_x_continuous(minor_breaks = c(0.1, 0.2, 0.3, 0.4)) +
...
# label axes
labx <- ggdraw() + draw_label("Capture rate", size = 10)
...
pdf(file=paste("./figures/allModels_N500Estim_v2.pdf", sep=""), 
