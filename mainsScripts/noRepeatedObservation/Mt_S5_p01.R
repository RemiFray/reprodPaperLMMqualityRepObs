...
bias_plan <- data.frame(a = rep(alpha_test, each = 100),
                        iter = rep(1:100, length(alpha_test)))

burnin <- 50000
nthin <- 50
niter <- 50000*nthin+burnin


# ------- File management ------ ----

tmpDir <- paste0("tmp/", modelType, "_S", S, "_p", p, "_noPrior")
...
for(line in 175:nrow(bias_plan)){
